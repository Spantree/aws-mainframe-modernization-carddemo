...
| 43 REDEFINES — same memory area interpreted as different types | High | Map each REDEFINES to a Java discriminated union or sealed class |
| SYNCPOINT = distributed transaction commit | High | Replace with Spring `@Transactional` + idempotency key |
| HANDLE ABEND — catch-all abend handler | Medium | Replace with `@ControllerAdvice` exception handler |
...
**Recommendation:** **Manual rewrite.** Automated COBOL-to-Java tools will produce unmaintainable output for this program. Decompose into:
1. `AccountService.updateAccount()` — business logic + validation
2. `AccountController` — REST endpoint
...
| 8 REDEFINES — card record interpreted multiple ways | High | Map to Java record hierarchy; verify with data-model.md |
| Dual-file write (CARDFILE + CARDXREF must stay in sync) | High | Single `@Transactional` service wrapping both `save()` calls |
...
| Risk | Severity | Mitigation |
|---|---|---|
| VSAM STARTBR/READNEXT/ENDBR browse = cursor-based pagination | High | Replace with Spring Data `Pageable` + JPA keyset pagination |
...
| **POINTER to PSA/TCB/TIOT** — mainframe OS control block addressing | **Critical** | Determine what data is accessed; replace with Java alternatives or eliminate the need entirely |
| 2D OCCURS arrays for table formatting | High | Convert to Java 2D array or List<List<>> |
| HTML generation in COBOL | Medium | Replace with Thymeleaf or FreeMarker template |
| Tightly coupled to CBSTM03B (I/O dispatcher) | Medium | Merge into one Java class; eliminate subprogram boundary |

**Recommendation:** **Manual rewrite required.** This is the only program in the codebase using both ALTER and POINTER. The POINTER-based MVS control block access must be investigated — if it reads system timing data (STCK), replace with `System.currentTimeMillis()`. If it reads job/task metadata, determine if that data is needed in the Java version.

**Special note:** CBSTM03A generates HTML. This is rare in COBOL. The HTML template logic should be extracted to a proper template engine, making the Java replacement significantly simpler than the COBOL source.
...
| Risk | Severity | Mitigation |
|---|---|---|
| **CICS START** — fires a batch transaction from an online screen | **High** | Replace with Spring Batch `JobLauncher` triggered by REST endpoint; use JMS/Kafka to decouple online from batch |
...
| CICS START → started transaction has no synchronous return path | Medium | Use async job status polling endpoint in Java |
...
**Recommendation:** The CICS START pattern represents an **architectural boundary** — online-to-batch async initiation. The Java replacement requires both a UI (POST /reports/initiate) and a batch job runner (Spring Batch Job). Design the async boundary before implementing CORPT00C.
...
| Risk | Severity | Mitigation |
|---|---|---|
| Updates account balances — financial mutation | Critical | Require shadow run: mainframe + Java produce identical ACCTFILE state |
| Rejects invalid transactions to REJECT-FILE | High | Rejection criteria must be fully documented and tested |
| No error recovery in COBOL (abend = rerun) | Medium | Implement idempotent Java job with restart capability |

### COBIL00C — Composite 2.90 | ⚠️ FINANCIAL CRITICAL + VSAM ALT-INDEX

| Risk | Severity | Mitigation |
|---|---|---|
| Bill payment — financial mutation (BC=5) | Critical | `@Transactional` + idempotency key (prevent double-posting) |
...
| Risk | Severity | Mitigation |
|---|---|---|
| Interest calculation — must produce exact decimal results | Critical | Use `BigDecimal` throughout; validate against mainframe results on historical data |
...
| Risk | Severity | Mitigation |
|---|---|---|
| Application-level auth vs VSAM (no RACF/SSO) | High | Replace with Spring Security + BCrypt-hashed passwords in PostgreSQL |
...
| Risk | Severity | Mitigation |
|---|---|---|
| IBM MQ PUT — publishes account inquiry events | High | Document MQ message format; preserve message schema in Java Spring Integration |
...
| COPAUS2C has EXEC SQL + IMS in same program | High | Mixed data source requires transactional coordination in Java |
...
| CBACT04C | 1.85 | Interest calculation — BigDecimal precision | High | Wave 3 (shadow run) |
...
|---|---|---|
| COBDATFT (Assembler) | CBACT01C | Reimplement as Java `DateFormatter` utility |
| MVSWAIT (Assembler) | COBSWAIT | Replace with `Thread.sleep()` |
...
| CBSTM03B must migrate with CBSTM03A | CBSTM03A (Wave 8) | Merge both into one Java report service |
| CORPT00C async job framework | Downstream batch reports | Design async job boundary (Spring Batch + JMS/Kafka) before Wave 7 |
...
|---|---|---|---|
| EBCDIC → UTF-8 conversion scope | HIGH | All PIC X fields stored in IBM CP037 EBCDIC. Sort order differs (EBCDIC lowercase sorts before uppercase). Any SQL ORDER BY or Java comparison on character fields may produce different results. | Convert all PIC X fields during VSAM export; validate sort-sensitive queries against both orderings |
...
| COMP field range under TRUNC(STD) | MEDIUM | `PIC 9(9) COMP` max is 999,999,999 (not Integer.MAX_VALUE 2,147,483,647). If TRUNC(BIN) was used instead, ranges differ. | Verify compile JCL for TRUNC option in every batch program; add range validation in Java setters |
...
| OCCURS DEPENDING ON (variable-length COMMAREA) | LOW | COPAUS1C uses OCCURS DEPENDING ON for variable-length DFHCOMMAREA. Java equivalent needs dynamic array handling. | Use `List<T>` with max-size validation; test with boundary-size COMMAREA |
...
| Risk | Severity | Detail | Mitigation |
|---|---|---|---|
| GDG ordering not enforced in code | HIGH | Batch pipeline POSTTRAN→INTCALC→COMBTRAN→TRANREPT must execute in order; order is enforced by GDG generation numbers. No code enforces this — only operational practice. | Model as Spring Batch job graph with explicit `after()` dependencies; or use AWS Step Functions with sequential state machine |
| CORPT00C → JOBS TD Queue (internal reader) | HIGH | Only CICS-to-batch bridge. EXEC CICS START + WRITEQ TD JOBS submits JCL to internal reader. No Java equivalent without async job framework. | Design Spring Batch JobLauncher + REST endpoint before Wave 7; test end-to-end before CORPT00C migration |
| IBM MQ message contract (COACCT01) | HIGH | COACCT01 publishes account inquiry events to MQ queue. Downstream consumers outside this codebase may depend on the MQ message schema. If schema changes, downstream systems break. | Identify all MQ queue consumers before migration; document message schema; preserve format in Spring Integration |
...
| CICS START async (CODATE01) | MEDIUM | CODATE01 uses CICS START + RETRIEVE pattern for asynchronous date service. No Java equivalent without event-driven framework. | Replace with Spring @Async + JMS/Kafka consumer; or eliminate if date service is no longer needed in the Java target |
| DALYTRAN external input feed | MEDIUM | CBTRN02C and CBTRN01C read DALYTRAN (daily transaction file) from external feed. Source of this feed not visible in this codebase. | Identify DALYTRAN producer; design Java equivalent (event stream / file drop) before Wave 3 |
...
| Risk | Severity | Detail | Mitigation |
|---|---|---|---|
| No test harnesses in source tree | HIGH | Zero unit tests, regression test data, or test harness found in `app/`. Migration quality cannot be measured without a test baseline. | Establish test data set and dual-harness (GnuCOBOL + Java) before Wave 1 begins |
| COBOL tribal knowledge | HIGH | COACTUPC (4,236 lines, no comments) contains undocumented business rules. If no COBOL SME is available, validation logic may be misunderstood. | Identify and retain COBOL SME for Wave 7–8 program reviews; budget for COBOL reading training for Java developers |
| Mainframe access for parallel run | MEDIUM | Parallel run requires simultaneous mainframe access and Java environment. Mainframe access hours, change windows, and cost must be pre-arranged with operations. | Confirm parallel run access agreement with mainframe operations team before Phase 3 |
| Team Java/Spring skill gaps | MEDIUM | Migration team must have Java 21, Spring Boot 3, Spring Batch, and PostgreSQL skills. Missing skills delay translation and reduce quality. | Conduct skills assessment before Wave 0; address gaps with training or consulting augmentation |
...
| Post-migration maintainers not identified | MEDIUM | The Java system needs maintainers after migration. If they are not identified early, knowledge transfer does not happen during migration. | Identify post-migration Java team before Wave 4; include them in code reviews from Wave 1 |
